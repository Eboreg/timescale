...
const exampleCount = Object.values(timescaleData).flatMap((m) => m.examples || []).length;
    ...
    if (scale) {
        ...
        if (scaleLegends) {
            ...
            if (width < 1) {
                ...
                if (magnitude >= 0)
                    longformSeconds.textContent = `(100 ${zeros.join(" ")} s)`;
                else if (magnitude == -3)
                    longformSeconds.textContent = "(0,1 s)";
                else
                    longformSeconds.textContent = `(0,${zeros.join(" ")} 1 s)`;
            }
            else if (width < 10) {
                scaleLegend.textContent = `10 ${timescaleData[magnitude].plural}`;
                scaleLegend.style.width = longformSeconds.style.width = `${width * 10}%`;
                if (magnitude >= 0)
                    longformSeconds.textContent = `(10 ${zeros.join(" ")} s)`;
                else if (magnitude == -3)
                    longformSeconds.textContent = "(0,01 s)";
                else
                    longformSeconds.textContent = `(0,${zeros.join(" ")} 01 s)`;
            }
            else {
                scaleLegend.textContent = `1 ${timescaleData[magnitude].singular}`;
                scaleLegend.style.width = longformSeconds.style.width = `${width}%`;
                if (magnitude >= 0)
                    longformSeconds.textContent = `(1 ${zeros.join(" ")} s)`;
                else if (magnitude == -3)
                    longformSeconds.textContent = "(0,001 s)";
                else
                    longformSeconds.textContent = `(0,${zeros.join(" ")} 001 s)`;
            }
...
function zoomBy(zoomLevel, delta) {
    ...
    if (magnitude != undefined) {
        ...
        if ((delta > 0 && timescaleData[magnitude] != undefined) ||
